    ...
    switch ($_GET['action'])
    {
        ...
        case '1':
            if ($_POST['kod'])
            {
                ...
                if (is_file('razdely /' . $name32 . '1.dat')) // if the section is created
                
                {
                    echo 'Section already created <br> <a href="clickrazdel.php?action=2"> Edit </a> <br/>';
                }
                else
                {
                    if ($_POST['kod1'])
                    ...
                    //////////////////////////
                    $fp = fopen('razdely /' . $name32 . '1.dat', "a +"); // Create file
                    flock($fp, LOCK_EX);
...
<select class = "buttom" size = "1" width = "70" name = "topath"> <option value = "' . $setup[' path '] . ' /"> ./ </option> ';
                while ($item = mysql_fetch_array($dirs))
                {
                    ...
                    {
                        if (strpos($value, '!') ! == false) $name = trans($value);
                        else $name = $value;
                        $option = $option . $name . '/';
                    }
...
<br/> <br/> <font color = "red"> * </font> -obezatelny required <br/>
        ...
        case '2':
            if ($_GET['razdel'])
            {
                ...
                if (is_file('razdely /' . $razdel . '1.dat')) // if the section is created
...
Select: </div> <table style = "margin: 0px; border-bottom: 1px solid # 86929d;" width = "100%" cellspacing = "1"> ';
                while ($item = mysql_fetch_array($dirs))
                {
                    ...
                    ############### Check for the existence of a section to a directory
                    $name32 = $item['path'];
                    $name32 = str_replace('files /', '', $name32);
                    $name32 = str_replace('/', ';', $name32);
                    $name32 = str_replace('_', '', $name32);
                    if (is_file('razdely /' . $name32 . '1.dat')) // if the section exists
                ...
                if ($su4 == NULL) echo 'No sections created. <br/> <a href="clickrazdel.php?action=1"> Write </a> <br/>';
        ...
        case '3':
            if ($_POST['tex2'])
            {
                if (is_file('razdely /' . $_GET[' razdel '] . ' 1.dat ')) // if the section is created
                
                {

                    ////////////////////////// delete all unnecessary from the file
                    ...
                    fclose($fp);
                    ////////////////////////////////
                    $fp = fopen('razdely /' . $_GET[' razdel '] . ' 1.dat ', "a +"); // Create file
                    flock($fp, LOCK_EX);
                    fputs($fp, $_POST['tex2']);
                    fflush($fp);
                    flock($fp, LOCK_UN);
...
Delete section? <a href="clickrazdel.php?action=4&yes=1&razdel=' . $_GET['razdel' > ] . '"> Y </a> / <a href="clickrazdel.php?action=2"> N < / a> </div> ';
            }
            if (is_file('razdely /' . $_GET[' razdel '] . ' 1.dat ') and $_GET[' yes'] == 1) // if the section is created
