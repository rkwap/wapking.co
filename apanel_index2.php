...
echo ' <div class="menu"><a href="apanel_index2.php">Downloads</a>&raquo;'.$put.'</div><div class="a">';
//------------------------------------------------------------------------------------------
if($setup['eval_change']==1) $eval = ',<a href="apanel_index2.php?sort=eval">rating</a>'; else $eval='';
if($sort == 'name') $sortlink = '<a href="apanel_index2.php?sort=data">date</a>,<a href="apanel_index2.php?sort=size">size</a>,<a href="apanel_index2.php?sort=load">popularity</a>'.$eval;
if($sort == 'data') $sortlink = '<a href="apanel_index2.php?sort=name">name</a>,<a href="apanel_index2.php?sort=size">size</a>,<a href="apanel_index2.php?sort=load">popularity</a>'.$eval;
if($sort == 'size') $sortlink = '<a href="apanel_index2.php?sort=data">date</a>,<a href="apanel_index2.php?sort=name">name,<a href="apanel_index2.php?sort=load">popularity</a></a>'.$eval;
if($sort == 'load') $sortlink = '<a href="apanel_index2.php?sort=data">date</a>,<a href="apanel_index2.php?sort=name">name,<a href="apanel_index2.php?sort=size">size</a>'.$eval;
if($sort == 'eval' and $setup['eval_change']==1) $sortlink = '<a href="apanel_index2.php?sort=data">date</a>,<a href="apanel_index2.php?sort=name">name,<a href="apanel_index2.php?sort=size">size</a>,<a href="apanel_index2.php?sort=load">popularity</a>';
...
echo $ico.'<strong><a href="apanel_index2.php?id='.$file_info['id'].'">'.$file_info['name'].'</a></strong>('.$allinfolder.')[<font color="#0080FF"><a href="apanel_scan2.php?id='.$file_info['id'].'">Refresh</a></font>]';
...
$asd= $page - 2;
$asd2= $page + 3;
if($asd<$all && $asd>0 && $page>3) echo '<a href="apanel_index2.php?id='.$id.'&amp;page=1">1</a> ... ';
for($i=$asd; $i<$asd2;$i++)
{
if($i<$all && $i>0)
{
...
else echo '<a href="apanel_index2.php?id='.$id.'&amp;page='.$i.'">'.$i.'</a> ';
}
}

if ($i <= $pages)
{
if($asd2<$all) echo ' ... <a href="apanel_index2.php?id='.$id.'&amp;page='.$pages.'">'.$pages.'</a>';
}
echo '<br>';
//------------------------------------------------------------------------------------------
...
<form action="apanel_index2.php?id='.$id.'" method="post">
...
else echo '[<a href="apanel_index2.php?onpage='.$i.'&amp;id='.$id.'">'.$i.'</a>]';
}
echo '<br>';
}
//------------------------------------------------------------------------------------------
...
list($msec,$sec)=explode(chr(32),microtime());
echo '<div class="title">'.round(($sec+$msec)-$HeadTime,4).' сек.<br>[&copy;www.wapscripts.info]</div></body></html>';
